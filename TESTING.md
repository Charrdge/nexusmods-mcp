...
Ожидаемый вывод включает `OK initialize`, строку `OK tools/list contains all 17 tools`, вызовы tools и в конце `ALL_OK`. Для `nexus_search_mods` при сбое GraphQL возможно предупреждение `WARN` (smoke не падает только на этом шаге); остальные tool-вызовы должны завершиться без `fatalf`.
...
| В IDE не хватает tool (ожидается 17) | Пересобери `docker build -t nexus-mcp:local .` или `go build`, перезапусти MCP; см. [docs/MCP.md](docs/MCP.md) |
